...
- Reset Game: clears board/history/status, sets current player to X, keeps scoreboard unchanged, 
  reuses the same game id, and keeps the game's existing mode unchanged
...
- Mode is set once at game creation and is immutable for that game's lifetime — it cannot 
  be changed via Reset Game. Starting a New Game (e.g. to switch modes) creates a fresh game 
  session with a new game id, distinct from Reset Game
